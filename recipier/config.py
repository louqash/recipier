...
class TaskConfig:
    ...
    diet_profiles: dict[str, str] = field(default_factory=lambda: {})  # Maps user names to diet profiles (e.g., {"John": "high_calorie", "Jane": "low_calorie"})
    ...
    def validate(self) -> None:
        ...
        if self.user_mapping:
            users_without_diet = [
                user for user in self.user_mapping.keys()
                if user not in self.diet_profiles
            ]
